...
public:
  ...
  // ── BFT Consensus (v14+) ──────────────────────────────────────────────────
  std::shared_ptr<BftConsensus> bft_consensus() const { return m_bft_consensus; }
  void set_broadcast_bft_propose_func(std::function<void(const NOTIFY_MEVATRUST_PROPOSE_BLOCK::request&)> fn);
  void set_broadcast_bft_vote_func(std::function<void(const NOTIFY_MEVATRUST_VOTE::request&)> fn);
  void set_broadcast_bft_new_view_func(std::function<void(const NOTIFY_MEVATRUST_NEW_VIEW::request&)> fn);
  void set_bft_commit_callback(std::function<void(uint64_t, const std::vector<uint8_t>&)> fn);
  void set_bft_block_proposer_func(std::function<std::optional<NOTIFY_MEVATRUST_PROPOSE_BLOCK::request>(uint64_t, uint64_t)> fn);
  ...
  crypto::secret_key get_node_sk() const { std::lock_guard<std::mutex> lk(m_lock); return m_node_sk; }
  ...
  // ── Proposer Election (Score-weighted, deterministic) ────────────────────
  struct EligibleProposer {
    crypto::public_key node_pubkey;
    crypto::hash node_id;
    float total_score;
  };

  std::vector<EligibleProposer> get_eligible_proposers() const;
  crypto::public_key next_proposer(uint64_t height, const crypto::hash& prev_id) const;
  bool verify_proposer_for_block(uint64_t height, const crypto::hash& prev_id,
                                 const crypto::public_key& claimed_proposer) const;

  uint64_t proposer_min_score() const { return m_proposer_min_score; }
  void set_proposer_min_score(float s) { m_proposer_min_score = s; }
...
private:
  ...
  float m_proposer_min_score{0.5f};
  ...
  // ── BFT state ─────────────────────────────────────────────────────────────
  std::shared_ptr<BftConsensus> m_bft_consensus;
};
