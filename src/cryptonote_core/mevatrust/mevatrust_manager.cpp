...
bool MevaTrustManager::init(const std::string& data_dir, network_type nettype, uint8_t hf)
{
  ...
  try {
    ...
    // ── BFT Consensus (v14+) ──────────────────────────────────────────────────
    m_bft_consensus = std::make_shared<BftConsensus>(this);
    MINFO("[MevaTrustManager] BFT Consensus engine created");
...
void MevaTrustManager::trigger_distribution(uint64_t height) {
  ...
  // Single-signer mode: the current proposer signs with its own key.
  // When the P2P commit protocol is operational, T-of-N partial signatures
  // from distinct proposers will be collected via NOTIFY_MEVATRUST_VOTE messages.
  cryptonote::mevatrust::frost::FrostSignature frost_sig;
  {
    auto outputs = mevatrust::build_distribution_outputs(m_resolved_rewards, pool_balance);
    ...
    if (m_node_key_set) {
      // Generate standard Schnorr signature with the node's key
      crypto::signature local_sig;
      crypto::generate_signature(msg_hash, m_node_pk, m_node_sk, local_sig);
      // Monero signature format: (c, r) where s*G = R + c*Y
      // FROST format: R = nonce commitment, z = s
      memcpy(&frost_sig.R, &local_sig.c, 32);
      memcpy(&frost_sig.z, &local_sig.r, 32);
      frost_sig.msg_hash = msg_hash;
      MINFO("[FROST] Real signature generated with node key h=" << height);
    } else {
      // Node key not set — cannot sign (block will be rejected by validators)
      MWARNING("[FROST] No node key set — pool distribution will be invalid h=" << height);
...
// ============================================================================
// Proposer Election — Score-Weighted Deterministic Selection
// ============================================================================

std::vector<MevaTrustManager::EligibleProposer>
MevaTrustManager::get_eligible_proposers() const
{
  std::vector<EligibleProposer> result;
  if (!m_node_registry || !m_mevatrust_engine) return result;
  auto nodes = m_node_registry->get_active_nodes();
  for (const auto& n : nodes) {
    if (!n.is_validator) continue;
    if (n.status != NodeStatus::ACTIVE) continue;
    auto score = m_mevatrust_engine->calculate_node_score(n.node_id, 0);
    if (score.total_score < m_proposer_min_score) continue;
    EligibleProposer ep;
    ep.node_pubkey = n.node_pubkey;
    ep.node_id = n.node_id;
    ep.total_score = score.total_score;
    result.push_back(std::move(ep));
  }
  return result;
}

crypto::public_key MevaTrustManager::next_proposer(
  uint64_t height, const crypto::hash& prev_id) const
{
  auto eligible = get_eligible_proposers();
  if (eligible.empty()) return {};

  static constexpr uint64_t SCALE = 1000000;

  // Build cumulative integer weights from scores
  uint64_t total_weight = 0;
  std::vector<uint64_t> cumulative_weights;
  cumulative_weights.reserve(eligible.size());
  for (const auto& e : eligible) {
    uint64_t w = static_cast<uint64_t>(e.total_score * SCALE);
    if (w == 0) w = 1;
    total_weight += w;
    cumulative_weights.push_back(total_weight);
  }

  // Deterministic seed = H(prev_id || height || "mevatrust_proposer")
  std::string seed_data;
  seed_data.append(reinterpret_cast<const char*>(prev_id.data), 32);
  seed_data.append(reinterpret_cast<const char*>(&height), 8);
  const char domain[] = "mevatrust_proposer";
  seed_data.append(domain, sizeof(domain) - 1);
  crypto::hash seed = crypto::cn_fast_hash(seed_data.data(), seed_data.size());

  uint64_t seed_val;
  memcpy(&seed_val, seed.data, sizeof(seed_val));
  uint64_t pick = seed_val % total_weight;

  // Linear scan over cumulative weights
  for (size_t i = 0; i < cumulative_weights.size(); ++i) {
    if (pick < cumulative_weights[i]) return eligible[i].node_pubkey;
  }

  return eligible.empty() ? crypto::public_key{} : eligible.back().node_pubkey;
}

bool MevaTrustManager::verify_proposer_for_block(
  uint64_t height, const crypto::hash& prev_id,
  const crypto::public_key& claimed_proposer) const
{
  crypto::public_key expected = next_proposer(height, prev_id);
  if (expected == crypto::public_key{}) {
    // Nessun proposer eletto (es. nessun validator attivo) — skip
    return claimed_proposer == crypto::public_key{};
  }
  return expected == claimed_proposer;
}
...
void MevaTrustManager::set_broadcast_bft_propose_func(
    std::function<void(const NOTIFY_MEVATRUST_PROPOSE_BLOCK::request&)> fn) {
  if (m_bft_consensus) m_bft_consensus->set_broadcast_propose_func(std::move(fn));
}

void MevaTrustManager::set_broadcast_bft_vote_func(
    std::function<void(const NOTIFY_MEVATRUST_VOTE::request&)> fn) {
  if (m_bft_consensus) m_bft_consensus->set_broadcast_vote_func(std::move(fn));
}

void MevaTrustManager::set_broadcast_bft_new_view_func(
    std::function<void(const NOTIFY_MEVATRUST_NEW_VIEW::request&)> fn) {
  if (m_bft_consensus) m_bft_consensus->set_broadcast_new_view_func(std::move(fn));
}

void MevaTrustManager::set_bft_commit_callback(
    std::function<void(uint64_t, const std::vector<uint8_t>&)> fn) {
  if (m_bft_consensus) m_bft_consensus->set_commit_callback(std::move(fn));
}

void MevaTrustManager::set_bft_block_proposer_func(
    std::function<std::optional<NOTIFY_MEVATRUST_PROPOSE_BLOCK::request>(uint64_t, uint64_t)> fn) {
  if (m_bft_consensus) m_bft_consensus->set_block_proposer_func(std::move(fn));
}
  ...
  void MevaTrustManager::set_node_key(const crypto::secret_key& sk,
    ...
    if (m_bft_consensus)
      m_bft_consensus->set_node_key(sk, pk);
  }
