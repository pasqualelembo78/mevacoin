...
std::vector<BadgeSystem::BadgeStatistics> BadgeSystem::get_badge_statistics() {
    std::vector<BadgeStatistics> result;
    std::lock_guard<std::mutex> lk(cache_lock_);
    if (badge_cache_.empty()) return result;

    std::map<uint8_t, BadgeStatistics> stats_by_type;
    uint32_t total_nodes = static_cast<uint32_t>(badge_cache_.size());

    for (const auto& kv : badge_cache_) {
        for (const auto& b : kv.second) {
            auto& st = stats_by_type[static_cast<uint8_t>(b.type)];
            st.type = b.type;
            st.total_awarded++;
            st.holders.push_back(kv.first);
            if (b.is_active) st.total_active++;
            else st.total_revoked++;
        }
    }

    for (auto& kv : stats_by_type) {
        kv.second.name = badge_type_to_string(kv.second.type);
        kv.second.percentage_of_nodes = total_nodes > 0
            ? 100.0f * static_cast<float>(kv.second.holders.size()) / static_cast<float>(total_nodes)
            : 0.0f;
        result.push_back(kv.second);
    }

    std::sort(result.begin(), result.end(),
        [](const BadgeStatistics& a, const BadgeStatistics& b) {
            return a.total_active > b.total_active;
        });
    return result;
}
