...
struct ActivityEntry {
  std::string type;         // "block_relay", "tx_relay", "peer_update", "challenge"
  uint64_t timestamp;
...
private:
  ...
  std::map<std::string, std::vector<ActivityEntry>> activity_history_;
  std::map<std::string, MevaTrustScoreSnapshot> score_cache_;
  std::map<uint64_t, std::map<std::string, MevaTrustScoreSnapshot>> historical_scores_;
  ...
  // Configurable decay factor for historical score weighting (0.0–1.0)
  float decay_factor_;
  ...
  void apply_decay(uint64_t current_height);

  void track_activity(const crypto::hash& node_id, const std::string& type);
