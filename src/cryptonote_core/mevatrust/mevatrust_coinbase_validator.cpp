  ...
  std::string&       error_msg)
{
  ...
  // 7. Validate proposer declaration (tag 0xAC)
  {
    tx_extra_mevatrust_proposer prop;
    if (mevatrust::parse_mevatrust_proposer_from_tx(miner_tx, prop)) {
      if (prop.height != height) {
        error_msg = "Proposer height mismatch at h=" + std::to_string(height);
        MERROR(error_msg);
        return false;
      }
      if (!pm->verify_proposer_for_block(height, prev_id, prop.proposer_pubkey)) {
        error_msg = "Invalid proposer for block at h=" + std::to_string(height);
        MERROR(error_msg);
        return false;
      }
      MDEBUG("Proposer validated at h=" << height);
    } else {
      MDEBUG("No proposer tag 0xAC at h=" << height << " (pre-activation or no validators)");
    }
  }
