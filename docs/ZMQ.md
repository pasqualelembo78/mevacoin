# The Current/Future Status of ZMQ in Mevacoin

## ZMQ Pub/Sub
Client `ZMQ_SUB` sockets must "subscribe" to topics before it receives any data.
This allows filtering on the server side, so network traffic is reduced. Mevacoin
...
matching supported by both Mevacoin and ZMQ. The `format`, `context` and `event`
...
The Mevacoin daemon will ensure that events prefixed by `chain` will be sent in
...
chain instead of the last block. The Mevacoin daemon also ensures that
...
The Mevacoin daemon will send a `txpool_add` pub exactly once for each
