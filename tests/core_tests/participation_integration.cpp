...
#include "cryptonote_core/participation/node_registry.h"
#include "cryptonote_core/participation/participation_engine.h"
#include "cryptonote_core/participation/participation_snapshot_antireplay.h"
#include "cryptonote_core/participation/snapshot_broadcaster.h"
#include "cryptonote_core/participation/participation_types.h"
#include "cryptonote_core/participation/participation_security.h"
#include "cryptonote_core/participation/participation_tx_parser.h"
...
// ===========================================================================
TEST(ParticipationIntegration, T1_NodeRegistrationWithUTXOProof)
{
  auto tmp = make_tmp("t1");
  auto reg = std::make_shared<cryptonote::participation::NodeRegistry>(tmp.string());
  ...
  cryptonote::participation::NodeRegistryEntry e{};
...
// ===========================================================================
TEST(ParticipationIntegration, T2_AntiSybilRejectWithoutProof)
{
  using namespace cryptonote::participation::security;
  std::string err;
...
// ===========================================================================
TEST(ParticipationIntegration, T3_UptimeTrackingBadgeAssignment)
{
  auto tmp = make_tmp("t3");
  auto eng = std::make_shared<cryptonote::participation::ParticipationEngine>(tmp.string());
  ASSERT_TRUE(eng->init()) << "T3: ParticipationEngine::init() fallita";

  using Thr = cryptonote::participation::BadgeThresholds;
...
// ===========================================================================
//  build_participation_snapshot_extra() serializza la snapshot in un blob
//  con tag 0xA2. parse_participation_snapshot_extra() deve ricostruirla
//  identica (tutti i campi: height, period, awards, badges, rewards).
// ===========================================================================
TEST(ParticipationIntegration, T4_RewardCoinbaseBlob)
{
  cryptonote::participation::tx_extra_participation_snapshot snap{};
  snap.height = 200;
  snap.period = 2;

  for (int i = 0; i < 2; i++) {
    cryptonote::participation::tx_extra_participation_snapshot::NodeAward a{};
  ...
  ASSERT_TRUE(cryptonote::participation::build_participation_snapshot_extra(snap, blob))
      << "T4: build_participation_snapshot_extra() fallita";
  ASSERT_FALSE(blob.empty()) << "T4: blob vuoto";
  ASSERT_EQ(blob[0], 0xA2) << "T4: tag blob non 0xA2";

  // Deserializzazione (round-trip)
  cryptonote::participation::tx_extra_participation_snapshot snap2{};
  ASSERT_TRUE(cryptonote::participation::parse_participation_snapshot_extra(blob, snap2))
      << "T4: parse_participation_snapshot_extra() fallita";
...
// ===========================================================================
TEST(ParticipationIntegration, T5_AntiReplayPostRestart)
{
  ...
  {
    cryptonote::participation::ParticipationSnapshotAntireplay ar;
...
// ===========================================================================
TEST(ParticipationIntegration, T6_Quorum3of5Consensus)
{
  cryptonote::participation::tx_extra_participation_snapshot snap{};
  snap.height = 500;
  snap.period = 5;
  ...
  // Test quorum normale
  {
    ...
    cryptonote::participation::SnapshotBroadcaster br;
    br.set_apply_func(
        [&](const cryptonote::participation::tx_extra_participation_snapshot&) -> bool {
          apply_count++;
          return true;
        });
  ...
  // Test voti duplicati dello stesso proposer
  {
    ...
    snap.height = 501;
    cryptonote::participation::SnapshotBroadcaster br2;
    br2.set_apply_func(
        [&](const cryptonote::participation::tx_extra_participation_snapshot&) -> bool {
          cnt2++;
          return true;
...
// ===========================================================================
TEST(ParticipationIntegration, T7_RebuildRegistryFromChain)
{
  ...
  std::vector<cryptonote::participation::ParticipationTxRecord>  records;

  // Fase 1: popola registry originale con 5 nodi
  {
    auto reg = std::make_shared<cryptonote::participation::NodeRegistry>(tmp_orig.string());
    ...
    for (int i = 0; i < 5; i++) {
      ...
      cryptonote::participation::ParticipationTxRecord r{};
      r.type       = cryptonote::participation::ParticipationTxType::NODE_REGISTRATION;
  ...
  {
    auto reg_new = std::make_shared<cryptonote::participation::NodeRegistry>(tmp_rebuilt.string());
...
// ===========================================================================
TEST(ParticipationIntegration, T8_IncentiveHistoryTransparency)
{
  ...
  for (int period = 1; period <= 5; period++) {
    ...
    cryptonote::participation::tx_extra_participation_snapshot::NodeAward award{};
    award.node_id = nid;
  ...
  for (int i = 0; i < static_cast<int>(chain_blobs.size()); i++) {
    cryptonote::participation::tx_extra_participation_snapshot snap{};
    ASSERT_TRUE(cryptonote::participation::parse_participation_snapshot_extra(
        chain_blobs[i], snap))
  ...
  // Verifica integrita' dato on-chain: manomissione deve produrre dati divergenti
  {
    ...
    if (tampered.size() > 15) {
      ...
      cryptonote::participation::tx_extra_participation_snapshot snap_bad{};
      const bool ok = cryptonote::participation::parse_participation_snapshot_extra(
          tampered, snap_bad);
      if (ok) {
