...
# a main offline mevacoind, does most of the tests
# a restricted RPC mevacoind setup with RPC payment
# two local online mevacoinds connected to each other
N_MEVACOINDS = 5

# 4 wallets connected to the main offline mevacoind
# 1 wallet connected to the first local online mevacoind
# 1 offline wallet
N_WALLETS = 7
...
mevacoind_base = [builddir + "/bin/mevacoind", "--regtest", "--fixed-difficulty", str(DIFFICULTY), "--no-igd", "--p2p-bind-port", "mevacoind_p2p_port", "--rpc-bind-port", "mevacoind_rpc_port", "--zmq-rpc-bind-port", "mevacoind_zmq_port", "--zmq-pub", "mevacoind_zmq_pub", "--non-interactive", "--disable-dns-checkpoints", "--check-updates", "disabled", "--rpc-ssl", "disabled", "--data-dir", "mevacoind_data_dir", "--log-level", "1", "--rpc-max-connections-per-private-ip", "100", "--rpc-max-connections", "100"]
mevacoind_extra = [
  ["--offline"],
...
wallet_base = [builddir + "/bin/mevacoin-wallet-rpc", "--wallet-dir", WALLET_DIRECTORY, "--rpc-bind-port", "wallet_port", "--rpc-ssl", "disabled", "--daemon-ssl", "disabled", "--log-level", "1", "--allow-mismatched-daemon-version"]
wallet_extra = [
...
for i in range(N_MEVACOINDS):
  command_lines.append([str(18180+i) if x == "mevacoind_rpc_port" else str(18280+i) if x == "mevacoind_p2p_port" else str(18380+i) if x == "mevacoind_zmq_port" else "tcp://127.0.0.1:" + str(18480+i) if x == "mevacoind_zmq_pub" else builddir + "/functional-tests-directory/mevacoind" + str(i) if x == "mevacoind_data_dir" else x for x in mevacoind_base])
  if i < len(mevacoind_extra):
    command_lines[-1] += mevacoind_extra[i]
  outputs.append(open(FUNCTIONAL_TESTS_DIRECTORY + '/mevacoind' + str(i) + '.log', 'a+'))
