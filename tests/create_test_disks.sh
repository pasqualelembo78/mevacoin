...
tmpdir=$(mktemp --tmpdir -d mevacointest.XXXXXXXX)
...
echo MEVACOIN_TEST_DEVICE_HDD=$(device_mountpoint "$tmpdir" "$dev_rot")

dev_ssd=$(create_device "$tmpdir")
bdev_ssd=$(block_device_path "$dev_ssd")
echo 0 | root_exec tee "$bdev_ssd/queue/rotational" >/dev/null
echo MEVACOIN_TEST_DEVICE_SSD=$(device_mountpoint "$tmpdir" "$dev_ssd")
