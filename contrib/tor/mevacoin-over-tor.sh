...
for dir in \
  ...
  "$DIR/../../build/Windows/master/release/bin"
do
  if test -x "$dir/mevacoind"
  then
    mevacoind="$dir/mevacoind"
    break
  fi
done
if test -z "$mevacoind"
then
  echo "mevacoind not found"
  exit 1
fi
echo "Found: $mevacoind"

TORDIR="$DIR/mevacoin-over-tor"
TORRC="$TORDIR/torrc"
...
if test "$ready" = 0
then
  echo "Error starting mevacoind"
  tail -n 400 "$HOME/.mevacoin/mevacoin.log" | grep -Ev stacktrace\|"Error: Couldn't connect to daemon:"\|"src/daemon/main.cpp:.*Mevacoin\ \'" | tail -n 20
  exit 1
fi
